    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int r = 0; r < N; r++) {
            for (int c = 1; c < N; c++) {
                ...
                // 가로 상태 (0)
                if (c > 0) {
                    dp[r][c][0] += dp[r][c - 1][0]; // 가로에서 오는 경우
                    dp[r][c][0] += dp[r][c - 1][2]; // 대각선에서 오는 경우
                }
                
                // 세로 상태 (1)
                if (r > 0) {
                    dp[r][c][1] += dp[r - 1][c][1]; // 세로에서 오는 경우
                    dp[r][c][1] += dp[r - 1][c][2]; // 대각선에서 오는 경우
                }
                
                // 대각선 상태 (2)
                if (r > 0 && c > 0 && grid[r][c - 1] == 0 && grid[r - 1][c] == 0) {
                    dp[r][c][2] += dp[r - 1][c - 1][0]; // 가로에서 오는 경우
                    dp[r][c][2] += dp[r - 1][c - 1][1]; // 세로에서 오는 경우
                    dp[r][c][2] += dp[r - 1][c - 1][2]; // 대각선에서 오는 경우
                }
            }
        }
        
        // 결과: (N-1, N-1)에 도달하는 모든 방향의 경우의 수 합산
